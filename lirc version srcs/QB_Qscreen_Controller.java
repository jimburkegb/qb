...
//            Platform.runLater( () -> { qd.SwitchAnimation.start( false ) ;  } ) ;  //  false -> transit DOWN
        // dbg.log( "QS Ctlr", "Animation is DOWN");
            qd.SwitchAnimation.start( false ) ;   //  false -> transit DOWN
         } 
        else 
        { 
            qd.PlayerAtTable = 1 ;
            ...
            qd.Player2Score_setStyle.update(  qd.box_unhighlight ) ;
            //
//            Platform.runLater( () -> { qd.SwitchAnimation.start( true ) ;  } ) ;   //  false -> transit UP 
        // dbg.log( "QS Ctlr", "Animation is UP");

            qd.SwitchAnimation.start( true ) ;    //  true -> transit UP 
            //
...
//    }
   ...
   @FXML
    void tfKeyEvent_IR(KeyEvent ke)
    {
        ...
        String chr = "", chr2 = "" ;
        //
//        if (qd.StandoffTimer.running) { return ; }  ; //  The 'standoff' period is a short time (half a                                          
//        qd.StandoffTimer.stop();                      //  sec) when a command will be ignored. This is to  
//        qd.StandoffTimer.start();                     //  prevent 'machine-gunning' the remote.
//
//      KeyCode  code ;
//      code = ke.getCode() ; 
        //
