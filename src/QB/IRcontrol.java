...
import static QB.QB_Qscreen_Controller.endMatch;
    ...
    //
        switch ( qCmd )
        {
            case "NUMBER_1":    // *****************  -1-  ZERO Scores  *************************
                                //  ZERO scores
                                qd.MsgBox.show("Scores ZEROED", 2);  
                                QB_Qscreen_Controller.initScores();                                 
                                break ;  
                                // 
            case "NUMBER_2":    // *****************  -2-  ??? ***********************************
                                ...
                                {
                                    qd.MsgBox.show("Advanced Scoring now OFF", 10);                               
                                    qd.AdvancedScoring = 0 ;
                                } else 
                                {
                                    qd.MsgBox.show("Advanced Scoring now ON", 10);                              
            ...
            case "NUMBER_3":    // *****************  -3-  Lock/Unlock Scoreboard   ********************************
                                //
                                if ((qd.MatchInProgress) && (!qd.ScoreboardLocked) )
                                {
                                   qd.MsgBox.show("Can't lock while match in progress", 3);  
                                } else
                                {
                                    if (qd.ScoreboardLocked)
                                    {
                                        // qd.ScoreboardLocked = false ;
                                        qd.GameOver = false ;
                                        qd.MatchInProgress = true ;
                                        qd.MsgBox.show("Scoreboard UNLOCKED", 3);                                        
                                    } else
                                    {
                                        qd.MsgBox.show("Scoreboard LOCKED", 3); 
                                        endMatch() ;
                                    }
//                                    if (qd.GameOver)
//                                    {   qd.MsgBox.show("Scoreboard unlocked", 3); } 
...
//                                    qd.GameOver = !qd.GameOver ;    //  FLIP the GameOver boolean 
            ...
            case "NUMBER_4":    // *****************  -4-  Show network info  ***********************************
                                //
                                qd.MsgBox.show(qd.piNAME + ";" + qd.piIP + ";" + qd.piMACADDR + ";" + qd.piSSID+ ";" + qd.piNI, 10);                               
                                break ;  
                                //
            case "NUMBER_5":    // *****************  -5-  SHUTDOWN  ***********************************
                                //
                                qboard.dbg.log( "exec_command", "SHUTOWN");
                                qd.MsgBox.show("!!!! SHUTTING DOWN !!!!", 5);  
                                try 
                                { 
            ...
            case "NUMBER_7":    // *****************  -7-  Exit application  ***********************************
                                //
                                qd.MsgBox.show("EXIT APPLICATION", 2); 
            ...
            case "NEXTPLAYER":  // ****************  -SWAP PLAYERS-  Swap Plain and Spot players  **********************
                                ...
                                qd.MsgBox.show("SWAPPING PLAIN AND SPOT", 2); 
                                QB_Qscreen_Controller.SwapPlainAndSpot();           //  Briefly show the points  
                                break ;  
                                //
